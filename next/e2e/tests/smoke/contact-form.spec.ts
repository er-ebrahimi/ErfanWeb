...
test.describe('Contact form', { tag: '@smoke' }, () => {
  ...
  test.describe('on a CMS page with contact section', () => {
    test('shows form fields and submit button', async ({ page }) => {
      ...
      const contactInput = page.getByRole('textbox', {
        name: 'شماره تماس یا ایمیل',
      });
      const messageInput = page.getByRole('textbox', { name: 'پیام' });

      // If the contact form is not on the homepage, this test is inconclusive
      test.skip(
        !(await contactInput.isVisible()),
        'Contact form not found on homepage'
      );
    ...
    test('submits successfully with valid data and mocked API', async ({
      page,
    }) => {
      await page.goto('/fa');
      await waitForPageStable(page);
      ...
      await contactPage.fillForm(
        CONTACT_FORM.valid.contact,
        CONTACT_FORM.valid.message
      );
