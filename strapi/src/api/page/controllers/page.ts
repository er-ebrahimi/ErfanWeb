...
function ensureFaqPopulate(populate: unknown): Record<string, any> {
  ...
  const dz =
    merged.dynamic_zone && typeof merged.dynamic_zone === 'object'
      ? { ...merged.dynamic_zone }
      : {};
  const on = dz.on && typeof dz.on === 'object' ? { ...dz.on } : {};
  const faqBranch =
    on['dynamic-zone.faq'] && typeof on['dynamic-zone.faq'] === 'object'
      ? { ...on['dynamic-zone.faq'] }
      : {};
  const faqPopulate =
    faqBranch.populate && typeof faqBranch.populate === 'object'
      ? { ...faqBranch.populate }
      : {};
