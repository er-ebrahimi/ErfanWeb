...
test.describe('Sitemap & SEO', { tag: '@api' }, () => {
  ...
  test('pages from sitemap have meta title, description, and canonical URL', async ({
    page,
  }) => {
    ...
    for (const url of urlsToCheck) {
      ...
      expect
        .soft(seo.description, `${path} is missing meta description`)
        .toBeTruthy();
      expect
        .soft(seo.canonical, `${path} is missing canonical URL`)
        .toBeTruthy();
    }
  });
});
